 ...
 *
 * This follows the SPA algorithm described in Reda, I.; Andreas, A. (2003):
 * Solar Position Algorithm for Solar Radiation Applications. NREL Report No.
 * TP-560-34302, Revised January 2008.
 *
 * This is <i>not</i> a port of the C code, but a re-implementation based on the
 * published procedure.
 */
public final class SPA {
    ...
    /**
     * Calculate topocentric solar position, i.e. the location of the sun on the
     * sky for a certain point in time on a certain point of the Earth's
     * surface.
     *
     * This follows the SPA algorithm described in Reda, I.; Andreas, A. (2003):
     * Solar Position Algorithm for Solar Radiation Applications. NREL Report
     * No. TP-560-34302, Revised January 2008. The algorithm is supposed to work
     * for the years -2000 to 6000, with uncertainties of +/-0.0003 degrees.
     *
     * @param date Observer's local date and time.
     * @param latitude Observer's latitude, in degrees (negative south of
     * equator).
     * @param longitude Observer's longitude, in degrees (negative west of
     * Greenwich).
     * @param elevation Observer's elevation, in meters.
     * @param deltaT Difference between earth rotation time and terrestrial time
     * (or Universal Time and Terrestrial Time), in seconds. See
     ...
     * @param pressure Annual average local pressure, in millibars (or
     * hectopascals). Used for refraction correction of zenith angle. If unsure,
     * 1000 is a reasonable default.
     * @param temperature Annual average local temperature, in degrees Celsius.
     * Used for refraction correction of zenith angle.
     * @return Topocentric solar position (azimuth measured eastward from north)
     */
    public static double[] calculateSolarPosition(final GregorianCalendar date, final double latitude,
        ...
        final Julian jd = new Julian(date, deltaT);
    ...
    /**
     * Calculate the times of sunrise, sun transit (solar noon), and sunset for
     * a given day. The calculation is based on the astronomical definition of
     * sunrise and sunset, using a refraction correction of -0.8333°.
     *
     * @param day GregorianCalendar of day for which sunrise/transit/sunset are
     * to be calculated. The time of day (hour, minute, second, millisecond) is
     * ignored.
     * @param latitude Observer's latitude, in degrees (negative south of
     * equator).
     * @param longitude Observer's longitude, in degrees (negative west of
     ...
     * <a href ="http://asa.usno.navy.mil/SecK/DeltaT.html">http://asa.usno.navy.mil/SecK/DeltaT.html</a>.
     * For the year 2015, a reasonably accurate default would be 68.
     * @return An array of 3 GregorianCalendar values corresponding to the time
     * of sunrise, sun transit, and sunset, respectively. Note that the values
     * for sunrise and sunset may be null: in this case, the sun stays above or
    ...
    public static GregorianCalendar[] calculateSunriseTransitSet(final GregorianCalendar day,
        ...
        final Julian jd = new Julian(dayStart, 0);
        ...
        for (int i = 0; i < alphaDeltas.length; i++) {
            Julian currentJd = new Julian(jd.getJulianDate() + i - 1, 0);
    ...
    private static double[] calculateTopocentricSolarPosition(final double p, final double t, final double phi,
        ...
        return new double[]{topocentricAzimuthAngle, topocentricZenithAngle};
    }
    ...
    private static double calculateTrueObliquityOfEcliptic(final Julian jd, final double deltaEpsilon) {
    ...
    private static double calculateApparentSiderealTimeAtGreenwich(final Julian jd, final double deltaPsi,
