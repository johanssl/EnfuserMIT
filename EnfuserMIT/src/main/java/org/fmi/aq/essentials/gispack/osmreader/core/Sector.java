...
public class Sector implements Serializable {
    ...
    private static final long serialVersionUID = 7278472293822776147L;//Important: change this IF changes are made that breaks serialization! 
    float[][] degs;
    static int max = 50;
    ...
    public byte getSector_fast(double h, double w, double degreeInterval) {
        double r = Math.sqrt(h * h + w * w);
        // double r = Math.pow(h*h+w*w, 0.5);
        h = 40 * (h / r);
        w = 40 * (w / r);

        return (byte) (this.degs[(int) h + max][(int) w + max] / degreeInterval);

    }

    public byte getSector_fastest(double h, double w, double degreeInterval, double r) {
        if (r <= 0) {
            return (byte) 0;
        }
        h = 40 * (h / r);
